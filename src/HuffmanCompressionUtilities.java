...
public class HuffmanCompressionUtilities {
	...
	public HuffmanCompressionUtilities() {
		...
		// including fio.
		PriorityQueue<HuffmanTreeNode> queue = new PriorityQueue<HuffmanTreeNode>(HuffmanTreeNode.compareWeightOrd);
		this.queue = queue;
		this.root = null;
	...
	int[] readFreqWeights(File inf) {
		...
		try {
			while((line = br.readLine()) != null) {
				...
				weights[Integer.parseInt(values[0])] = Integer.parseInt(values[1]);
			}		
			br.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	void initializeHuffmanQueue(boolean minimize) {
		for (int i=0; i<NUM_ASCII; i++) {
			if(!minimize || (minimize && weights[i] != 0)) {
				HuffmanTreeNode htn = new HuffmanTreeNode(i, weights[i]);
				this.queue.add(htn);
			}
		}
	...
	void buildHuffmanTree(boolean minimize) {
		...
		HuffmanTreeNode left, right, node;
		left = null;
		right = null;
		while(!queue.isEmpty()) {
			left = queue.poll();
			...
			if (queue.isEmpty()) {
				root = left;
				System.out.println("Done processing queue... root weight: " + root.getWeight());
				return;
			}
			right = queue.poll();
			weight += right.getWeight();
			node = new HuffmanTreeNode(weight, left, right);
			queue.add(node);
		}
	}
	...
	int decodeString(String binStr) {
		if(binStr.isEmpty()) {
			...
			return traverseTree(this.getTreeRoot(), binStr, 0);
		}
	}
